...
import trimesh
import numpy as np
from src.utils.data_utils import discretize, undiscretize
...
def patchified_mesh(mesh: trimesh.Trimesh, special_token=-2, fix_orient=True):
    ...
    while sum(unvisited):
        ...
        if (
            fix_orient
            and len(res) >= 2
            and not face_orient["{}-{}-{}".format(max_deg_vertex, res[0], res[1])]
        ):
            res = res[::-1]
...
def get_block_representation(
    ...
    """
    block_val = block_size**3
    offset_val = offset_size**3
    special_block_base = block_val + offset_val
    ...
    block_id = (
        block_id[:, 0] * block_size**2 + block_id[:, 1] * block_size + block_id[:, 2]
    )
    offset_id = coords % offset_size
    offset_id = (
        offset_id[:, 0] * offset_size**2
        + offset_id[:, 1] * offset_size
        + offset_id[:, 2]
    )
    offset_id += block_val
    block_coords = np.concatenate(
        [block_id[..., None], offset_id[..., None]], axis=-1
    ).astype(np.int32)
    ...
    codes = np.array(codes).astype(np.int32)
    sequence = codes
...
def BPT_serialize(
    ...
    sequence = patchified_mesh(mesh, special_token=special_token)
...
def decode_block(sequence, compressed=True, block_size=8, offset_size=16):
    ...
    if compressed:
        ...
        for token_id in range(len(sequence)):
            if block_val + offset_val > sequence[token_id] >= block_val:
                res.append([res_block, sequence[token_id]])
            elif block_val > sequence[token_id] >= 0:
...
def BPT_deserialize(sequence, block_size=8, offset_size=16, compressed=True):
    ...
    for i in range(len(sequence)):
        sub_seq = []
        if (
            special_block_base <= sequence[i] < special_block_base + block_val
            or i == len(sequence) - 1
        ):
            if i != 0:
                sub_seq = (
                    sequence[start_idx:i]
                    if i != len(sequence) - 1
                    else sequence[start_idx : i + 1]
                )
                if special_block_base <= sub_seq[0] < special_block_base + block_val:
                    sub_seq[0] -= special_block_base
                sub_seq = decode_block(
                    sub_seq,
...
if __name__ == "__main__":
    ...
    from data_utils import process_mesh, to_mesh
    import torch

    mesh_path = "/home/kemove/devdata1/zyx/BuildingGPT/dataset/Objaverse_500_Silksong-128/model/131_4abe1b63a22748a087d6dec8d98a378b.obj"
    trimesh.load(mesh_path).export("original.obj")
    vertices, faces = process_mesh(mesh_path, augment=False, transpose=False)
    mesh = to_mesh(vertices, faces, transpose=False)
    mesh.export("gt.obj")
